...
Nodes represent `Users`, `Questions`, `Answers`, `Comments`, and `Tags`.
...
button::Explore the data model[role=NX_EXPLORE_SEARCH,search=show me a graph]

[TIP]
====
You can double-click on nodes to expand them and see their properties.
====
...
Modify the query to find `User` and `Answer` nodes using the `PROVIDED` relationship.

The query will return this graph of `User` and `Answer` nodes.

image::{img}/user-provided-answer.svg[A graph of User and Answer nodes. There are clear clusters of users and answers.]

From the distribution of the graph, you can see that there are a few users who have provided many answers.
...
.Replace the `??????` to complete the query.
[source,cypher]
----
...
Challenge - Can you find comments made by `"cybersam"` on questions tagged with `"neo4j"`? 
...
Identify other users in the graph and find the shortest path between them.
