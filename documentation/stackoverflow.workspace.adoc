...
Every developer has a tab open https://stackoverflow.com/[Stack Overflow^], the massively popular question-and-answer site. 
This dataset explores users, questions, answers, comments, and tags related to the `neo4j`- and `cypher`- tagged questions on Stack Overflow.
...
Throughout the guide you'll find Cypher statements that you can execute by clicking on the play button.

If you want to modify a statement, click the arrow instead to copy the statement to the Cypher editor, where you can modify before executing.

== The Model

image::{img}/model.svg[width=100%]

button::Get the StackOverflow dataset[role=NX_IMPORT_LOAD,endpoint=https://neo4j-graph-examples.github.io/stackoverflow/data/stackoverflow-data-importer.zip]
...
Update this dataset using `apoc.load.json`.
 Run the query as-is to update or add a tag to the `tags` array to extend. 
...
Read More: https://neo4j.com/docs/apoc/current/import/load-json/#load-json-examples-stackoverflow[Import from StackOverflow API^]
...
If you want to, you can also materialize those relationships in the graph and then explore the data in the _Explore_ tab.
